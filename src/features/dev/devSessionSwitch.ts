...
export type SwitchResult = "switched" | "needs-login" | "error";
...
export async function switchSession(from: AppMode, to: AppMode): Promise<SwitchResult> {
  ...
  try {
    ...
    // Re-stash the rotated tokens (setSession refreshes them) and flip the local mode;
    // the app-state refresh re-derives everything else from get_my_group().
